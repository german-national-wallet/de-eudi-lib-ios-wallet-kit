...
extension OpenId4VciService {
	...
	private func getIssuerForWalletAppCompatibility(offer: CredentialOffer, nonce: String? = nil, useDpop: Bool? = nil, dpopKeyOptions: KeyOptions? = nil) async throws -> Issuer {
		...
		if useDpop ?? config.requireDpop {
			// The key id is derived from the credential issuer id, so every flow for this issuer
			// (PAR, resume, refresh) reuses the same DPoP-bound key without persisting the id.
